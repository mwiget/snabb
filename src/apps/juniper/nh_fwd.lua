...
function nh_fwd:push ()
  ...
  -- from service
  if output_wire then
    for _=1,math.min(link.nreadable(input_service), link.nwritable(output_wire)) do
  ...
  elseif output_vmx then
    ...
    -- to the vMX
    for _=1,math.min(link.nreadable(input_service), link.nwritable(output_vmx)) do
      local pkt = receive(input_service)
      transmit(output_vmx, pkt)
    end
  end
  ...
  -- from wire
  if input_wire then
    for _=1,math.min(link.nreadable(input_wire), link.nwritable(output_service)) do
...
--          print(description .. " learning ipv6 nh mac address " .. ethernet:ntop(self.next_hop_mac))
  ...
  elseif input_vmx then
    ...
    for _=1,math.min(link.nreadable(input_vmx), link.nwritable(output_service)) do
      local pkt = receive(input_vmx)
      transmit(output_service, pkt)
    end
