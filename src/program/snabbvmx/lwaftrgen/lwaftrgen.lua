...
local setup = require("program.snabbvmx.lwaftrgen.setup")
...
local function show_usage (exit_code)
   print(require("program.snabbvmx.lwaftrgen.README_inc"))
   main.exit(exit_code)
end
...
function run(args)
   ...
   if file_exists(conf_file) then
      ...
      if not file_exists(conf.lwaftrgen) then
         -- Search in main config file.
         conf.lwaftrgen = lib.dirname(conf_file).."/"..conf.lwaftrgen
      end
      if not file_exists(conf.lwaftrgen) then
         fatal(("lwaftrgen conf file '%s' not found"):format(conf.lwaftrgen))
      end
      lwconf = require('program.snabbvmx.lwaftrgen.conf').load_lwaftrgen_config(conf.lwaftrgen)
   ...
   local c = config.new()
   setup.lwaftrgen_app(c, conf, lwconf, sock_path)
   engine.configure(c)
